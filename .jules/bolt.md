...
## 2025-05-25 - Robust Gzip Compression in Flask
**Learning:** In Flask, implementing Gzip compression in `@app.after_request` requires careful handling of headers. Using `response.vary.add('Accept-Encoding')` is safer than direct assignment as it preserves existing `Vary` values (like 'Cookie'). Additionally, setting `Vary: Accept-Encoding` for all compressible mimetypes—even if not compressed in a specific instance—ensures downstream caches correctly handle content negotiation.
**Action:** Always use `response.vary.add` and ensure `Vary` is set for all eligible types to maintain cache integrity.
