...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // Performance Optimization: stop observing once animated to avoid re-triggering
    ...
    const toggleMobileMenu = (forceState) => {
        ...
        const isOpen = typeof forceState === 'boolean' ? forceState : !navLinks.classList.contains('active');
        ...
        if (menuIcon) {
            // Support both fa-times and fa-xmark if they exist in CSS
            const isXMark = menuIcon.classList.contains('fa-xmark');
            const closeClass = isXMark ? 'fa-xmark' : 'fa-times';
            menuIcon.classList.toggle('fa-bars', !isOpen);
            menuIcon.classList.toggle(closeClass, isOpen);
        }
    };
    ...
    // Keyboard accessibility: Close mobile menu or modal on Escape
    ...
    // 5. Handle Server-side Payment Notifications
...
function sendMessage() {
    ...
    if (!nameInput || !emailInput || !messageInput || !submitBtn || !responseBox) return;
